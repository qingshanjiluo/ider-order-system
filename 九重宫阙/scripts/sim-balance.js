...
﻿  // ================= 轮57 新增：乘区2 真源一致 + 单次修为奖励零头锁 =================
  ...
  if (repArg !== -1 && process.argv[repArg + 1]) {
    ...
    out.push('- 修为奖励量级已实测并上锁（轮57）：' + (EXP_REWARD_STATS ? 'routes 里 `exp:` 常量 ' + EXP_REWARD_STATS.count + ' 处，合计 ' + EXP_REWARD_STATS.total.toLocaleString() + ' = 最低境整境需求（' + Math.round(EXP_REWARD_STATS.minRealmFill).toLocaleString() + '）的 ' + (EXP_REWARD_STATS.total / EXP_REWARD_STATS.minRealmFill * 100).toFixed(2) + '%；单笔最大 ' + EXP_REWARD_STATS.max.toLocaleString() + '（' + EXP_REWARD_STATS.maxFile + '）= ' + (EXP_REWARD_STATS.max / EXP_REWARD_STATS.minRealmFill * 100).toFixed(2) + '%。' : '（零头锁未执行）') + '缩放前一次性成就独占 531,400（单笔最大 200,000 = 3.85% 整境），现已两轮收敛到成就合计 5.7 万。**零头锁**：单次 ≤0.5% 且全库常量合计 ≤2% 整境需求 ⇒ 以后往成就/任务里塞大额修为会直接跑红，"修为必须来自修炼"不再靠运气。');
        out.push(`\n\n## DSH-HEADLINE（机器读，勿手改）\n\n\`\`\`json\n` + JSON.stringify({ kind:'balance', passed: pass, failed: fail, maxDriftPercent: +(maxDrift*100).toFixed(3), targetBands: TARGET_RATIO, gongfa: REAL_GONGFA && { P2: REAL_GONGFA.P2._from, P2speed: REAL_GONGFA.P2.cultivationSpeed, P3: REAL_GONGFA.P3._from, P3speed: REAL_GONGFA.P3.cultivationSpeed }, expRewards: EXP_REWARD_STATS && { count: EXP_REWARD_STATS.count, total: EXP_REWARD_STATS.total, maxSingle: EXP_REWARD_STATS.max, singlePct: +(EXP_REWARD_STATS.total? EXP_REWARD_STATS.max/EXP_REWARD_STATS.minRealmFill*100:0).toFixed(3), totalPct: +(EXP_REWARD_STATS.total/EXP_REWARD_STATS.minRealmFill*100).toFixed(2) } }, null, 1) + `\n\`\`\`\n`);
