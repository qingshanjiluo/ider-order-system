...
t('已知未解决：kill 口径下合体以上应劫近乎必败（把坏消息锁住，防止被误当已通过）', () => {
  ...
  // 轮103：选敌从「池内 hp 中位数」改为「本境界综合战力最高（hp×攻击×防御）」。
  //   病因：用池子统计量当难度锚 → P7 新增 48 只怪把中位 hp 拉低，合体/渡劫应劫从必败变必胜（0%→100%）。
  //   改后：难度锚是"同级最难打的那只"，新增低档怪不会再削弱大限劫；
  //   要削弱必须显式改那只怪的数值 —— 改不动就藏不住，本锁因此可以维持严格阈值。
  assert.ok(t3 <= 0.35 || t5 <= 0.35,
    `合体以上应劫应近乎必败，实测 合体 ${(t3 * 100).toFixed(0)}% / 渡劫 ${(t5 * 100).toFixed(0)}% —— ` +
    `若变易，多半是 pickTribulationMonster 的选敌锚又被换回统计量（池子一变难度就漂）`);
});
