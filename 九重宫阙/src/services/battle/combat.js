...
class CombatService {
  ...
  getEntity(id, type, db) {
    ...
    if (type === 'character') {
      ...
      //
      //   v6（现在）：v5 的 95 分边界仍不单调 —— 90 走「平静」(def×1.05)，
      //       95 走「兴奋」(atk×1.05, spd×1.1) 却**丢掉了防御加成**，
      //       在该角色的属性分布下综合分 186 → 183，**掉下去了**。
      //
      //       根因是同一个：把高档做成"换成另一组乘数"而不是"在前一档基础上加码"。
      //       现在高档**保留低档的全部加成再叠加** —— 严格支配，单调性由构造保证，
      //       不再依赖具体角色的属性分布（这是关键：之前的版本在某个角色上
      //       碰巧单调，换个属性分布就破功，判据本身没保证）。
      ...
      if (Number.isFinite(numericMood)) {
        // 基线：≥40 的正常区间给稳定小幅防御加成（原有「平静」语义）
        if (numericMood >= 40) moodDefense = 1.05;

        if (numericMood >= 95) {
          // 极佳：在基线之上叠加攻速（原有「兴奋」语义），不替换
          moodAttack *= 1.05;
          moodSpeed *= 1.1;
