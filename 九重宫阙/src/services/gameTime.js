...
/** 有效寿元上限 = min(境界基础 + 加成 - 惩罚, 100万)；飞升为 null */
function effectiveLifespan(character) {
  ...
  return Math.max(1, Math.min(base + bonus - penalty, LIFESPAN_CAP));
}
...
function addLifespanBonus(character, years) {
  ...
  character.lifespan_bonus_years = (character.lifespan_bonus_years || 0) + years;
...
function shouldPassAway(character) {
  ...
  const cap = effectiveLifespan(character);
  return (character.age_years || 0) >= cap;
}
