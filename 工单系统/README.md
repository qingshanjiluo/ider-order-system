# 艾德尔修仙传 — 工单系统

赛博朋克风格的邀请积分购买工单平台，配合 GitHub Actions 实现全自动账号注册、刷怪、升级。

## 架构

```
┌─────────────┐     ┌──────────────┐     ┌───────────────┐
│  Cloudflare  │     │  Cloudflare  │     │  GitHub       │
│  Pages       │────▶│  Workers     │────▶│  Actions      │
│  (前端界面)   │     │  (API+D1数据库) │     │  (自动注册/刷怪) │
└─────────────┘     └──────────────┘     └───────────────┘
```

## 核心功能

| 模块 | 说明 |
|------|------|
| 用户系统 | 注册/登录（单IP限制）、等级体系（最高10级70%折扣） |
| 工单系统 | **仅「购买邀请积分」一种工单**：提交（邀请码+支付信息+优惠码）→ 审核 → 自动处理。修仙币支付即时扣款并**自动批准**，现金/灵石支付需管理员审核 |
| 邀请系统 | 生成邀请码、30%分成、邀请积分提现 |
| 客服机器人 | 工单状态查询、价格说明、常见问题 |
| 账号监控 | 实时显示账号等级、地图、技能、装备信息 |
| 自动注册 | GH Actions 定时扫描工单，自动注册账号（全金灵根） |
| 自动升级 | 每日健康检测，自动点击升级，120级停号 |
| 防封号 | 独立IP伪装、指纹轮换、智能分段暂停 |

## 部署

### 1. Cloudflare D1 数据库

```bash
npm install
npx wrangler d1 create ider-orders
npx wrangler d1 execute ider-orders --file=schema.sql
```

### 2. 部署 Worker

```bash
npx wrangler deploy
```

### 3. 配置 Secrets

在 GitHub 仓库设置 Secrets：
- `WORKER_URL` — 你的 Worker 地址（如 `https://ider-order.xxx.workers.dev`）
- `API_KEY` — Worker 中设置的 API_KEY

### 4. 初始化管理员

```sql
-- 在 D1 中设置管理员（level >= 99）
UPDATE users SET level = 99 WHERE username = 'admin';
```

## 工单类型

平台**只提供「购买邀请积分」一种工单**，其余类型（仙盟采集 / 试炼测试 /
每日试炼 / 传人派出 / 副本刷取）已于 v18 下线：前端不再提供入口，
`POST /api/orders` 以白名单拒绝写入，扫描器（`/api/gh/approved-orders`）
也不再下发这些类型。

| 付款方式 | 是否自动批准 | 说明 |
|----------|--------------|------|
| 修仙币（coin） | ✅ 是 | 提交时即时扣款，直接置为 `approved` 并开始处理；经验/邀请分成/套餐与人工审核完全一致 |
| 现金（微信） | ❌ 否 | 置为 `pending`，等待管理员审核 |
| 灵石 | ❌ 否 | 置为 `pending`，等待管理员审核 |

> 存量非邀请积分工单的清理脚本：`migration_v18_remove_other_order_types.sql`
> （先归档到 `orders_removed_archive`，再删除；含可选的修仙币退款语句）。

## GitHub Actions

| 工作流 | 触发 | 功能 |
|--------|------|------|
| `order-scan.yml` | 每6小时 / 手动 | 扫描审核通过的工单，自动注册账号 |
| `health-check.yml` | 每天2次 / 手动 | 检测账号状态，自动升级 |

## 定价

- 微信支付：1 元 = 120 邀请积分
- 灵石支付：100 万灵石 = 10 邀请积分
- 优惠码 + 等级折扣（最高70%）

## 用户等级

| 等级 | 条件 | 权益 |
|------|------|------|
| Lv.1 | 注册 | 基础价 |
| Lv.2 | 1单 | 解锁邀请系统 |
| Lv.3 | 3单 | 10%优惠 |
| Lv.4 | 5单 | 20%优惠 |
| Lv.5 | 10单 | 30%优惠 |
| Lv.10 | 100单 | 70%优惠 |

## 邀请分成

- 分享邀请码给好友注册
- 好友成交后获得该订单金额 **30%** 的邀请积分
- 邀请积分可提现或消费
