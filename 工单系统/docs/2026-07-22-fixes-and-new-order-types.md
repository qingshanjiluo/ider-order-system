...
> 状态：规划阶段
>
> ⚠️ **历史文档（2026-09-11 更新）**：本文档"第二部分 新增工单类型"中规划的
> 仙盟采集 / 试炼测试 / 每日试炼 / 传人派出 / 副本刷取 五种工单**已全部下线**。
> 平台现只保留「购买邀请积分」一种工单，相关前端表单、API 端点、GH Actions
> 脚本与工作流均已删除；存量数据见 `migration_v18_remove_other_order_types.sql`。
> 本文档仅作为历史实现记录保留，第一部分（修复项）仍然有效。
