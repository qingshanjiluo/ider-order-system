...
const MAX_ORDERS_PER_CALL = 30; // 单次最多处理工单数，防止超时（可重复调用）
...
export async function onRequest(context) {
  ...
  if (order_id) {
    ...
    // 找出所有账号数 > 订购数量的工单（按超额数量降序）
    const overOrders = await env.DB.prepare(
      `SELECT o.id as order_id, o.quantity,
        (SELECT COUNT(*) FROM game_accounts ga WHERE ga.order_id = o.id) as account_count
       FROM orders o
       WHERE o.quantity > 0
         AND (SELECT COUNT(*) FROM game_accounts ga WHERE ga.order_id = o.id) > o.quantity
       ORDER BY ((SELECT COUNT(*) FROM game_accounts ga WHERE ga.order_id = o.id) - o.quantity) DESC
       LIMIT ?`
  ...
  if (stmts.length === 0) return json({ ok: true, cleaned: 0, orders_processed: orderResults.length, message: '没有需要清理的超额账号' });

  await env.DB.batch(stmts);

  await logActivity(env, 0, user.id, 'admin_cleanup_excess', '一键清理超额: ' + totalCleaned + ' 个账号，处理 ' + orderResults.length + ' 个工单');
