...
async function getBotAnswer(question, env, user) {
  ...
  if (q.includes('多久') || q.includes('到账') || q.includes('时间') || q.includes('等待')) {
    return '[时] 工单审核通过后，预计 5 天内完成账号注册和升级。如果超过时间请联系管理员。\n\n[币] 修仙币充值：提交申请→管理员审核→自动生成兑换码→输入兑换码即刻到账';
  }
  ...
  if (q.includes('你好') || q.includes('嗨') || q.includes('在吗') || q.includes('hello')) {
    return '你好 ' + (user.username || '道友') + '！我是艾德尔工单助手 [AI]\n你可以问我：\n▸ "修仙币" - 了解坊市和充值\n▸ "我的订单状态" - 查看工单\n▸ "等级称号" - 了解称号系统\n▸ "优惠折扣" - 查看等级优惠\n▸ "邀请分成" - 邀请好友赚钱\n▸ "怎么充值" - 充值流程\n▸ "订单 #1" - 查看订单详情';
  }
