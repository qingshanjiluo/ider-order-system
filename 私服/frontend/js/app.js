...
import { initPanels, useSect, useAlchemy, useExchange, useAlliance, useChat, useDungeon, useDuel, useLeague, useBeast, useSectWar, useSettings, useAI, useAdmin } from './panels.js?v=20260821a';
import { useCultivation, initCultivation } from './cultivation.js?v=20260821a';
import { fmtSkillFull, fmtTechFull, fmtItemFull, fmtItemEffect, fmtSkillEffect, fmtTechEffect, fmtTechPassive, fmtAffix, fmtEquipmentDetail } from './formatters.js?v=20260821a';
import { getRealm, getRealmTier, getRealmStage, qualityColor, allianceRankName, qualityName, itemTierLine, formatNumber, DICTIONARY_ENTRIES, MINGTU_NODES, rollSpiritRoots, SPIRIT_ROOT_QUALITY_LABELS, alchemyGradeName } from './appShared.js?v=20260821a';
import { helpBaiyiHtml, helpAffixHtml } from './helpContent.js?v=20260821a';
import { getMapDrops } from './mapDrops.js?v=20260821a';
import { getCurrentMapInfo, buildMapTooltip, buildMapInfoLines } from './mapView.js?v=20260821a';
...
// 炼丹 UI 辅助函数（模块级，供模板直接调用）
function _alchemyGradeColor(g) {
  if (g <= 2) return '#4caf50';
  if (g <= 4) return '#42a5f5';
  if (g <= 6) return '#ab47bc';
  if (g <= 8) return '#ffa726';
  return '#ef5350';
}
function _alchemyGradeColorDark(g) {
  if (g <= 2) return '#2e7d32';
  if (g <= 4) return '#1565c0';
  if (g <= 6) return '#6a1b9a';
  if (g <= 8) return '#e65100';
  return '#b71c1c';
}
function _alchemyGradeBg(g) {
  if (g <= 2) return '#4a4';
  if (g <= 4) return '#48f';
  if (g <= 6) return '#a4f';
  if (g <= 8) return '#fa4';
  ...
  setup() {
    ...
    async function doSync() {
      ...
      try {
        ...
        if (!r.hasCharacter && !player.name) { view.value = 'create'; return; }
    ...
    async function doStateSync() {
      try {
        ...
        syncFailCount = 0; isOnline.value = true;
        if (!r.hasCharacter && !player.name) { view.value = 'create'; return true; }
        if (r.email_bound !== undefined) emailBound.value = !!r.email_bound;
        if (r.player && r.player.auto_battle_enabled !== undefined) {
          autoBattle.value = !!r.player.auto_battle_enabled;
          localStorage.setItem('auto_battle', autoBattle.value ? '1' : '0');
    ...
    function startSync() {
      ...
      syncTimer = setInterval(doPeriodicSync, 300000);
      document.addEventListener('visibilitychange', _onVisibilityChange);
      api.connectWs();
      api.onWsMessage(_handleWsMessage);
      chat.startPoll();
      // 一次性：旧材料转灵石迁移
      if (!localStorage.getItem('materials_migrated')) {
        api.migrateMaterials().then(r => {
          if (r && r.ok && r.converted > 0) {
            applyPlayer(r.player);
            showToast(r.msg || '旧材料已转换为灵石');
          }
          localStorage.setItem('materials_migrated', '1');
        }).catch(() => {});
      }
    }
    function stopSync() {
    ...
    const sectwar = useSectWar();
    const sett = useSettings();
    const ai = useAI();
    const admin = useAdmin();
    const cult = useCultivation(player);
    ...
    // create
    const createForm = reactive({ name:'', loading:false, error:'', roll:null });
    function rollCreateRoots() {
      createForm.roll = rollSpiritRoots();
      createForm.error = '';
    }
    watch(view, (v) => { if (v === 'create' && !createForm.roll) rollCreateRoots(); });
    async function doCreate() {
      if (!(createForm.name || '').trim()) { createForm.error = '请输入角色名称'; return; }
      if (!createForm.roll) { createForm.error = '请先参悟灵根'; return; }
      createForm.loading = true; createForm.error = '';
      try {
        const r = await api.createCharacter(createForm.name.trim(), {
          spirit_roots: createForm.roll.roots,
          spirit_root_quality: createForm.roll.quality,
          spirit_root_variant: createForm.roll.variant || '',
        });
    ...
    const activeModule = ref('combat'); // 当前激活的模块
    const MODULES = [
      { id:'cultivation', label:'修行', icon:'🧘', subTabs:['cultivation','techniques','skills','beast'] },
      { id:'combat', label:'战斗', icon:'⚔️', subTabs:['map','dungeon','trial','duel','league','sectwar'] },
      { id:'social', label:'社交', icon:'👥', subTabs:['chat','sect','alliance','mail'] },
      { id:'crafting', label:'制造', icon:'🔨', subTabs:['bailian','inventory','equipment','exchange'] },
      { id:'world', label:'世界', icon:'🌍', subTabs:['cave','disciple','shijieshu'] },
      { id:'more', label:'更多', icon:'⚙️', subTabs:['settings','dictionary','announcement'] },
    ];
    const currentModule = computed(() => MODULES.find(m => m.id === activeModule.value) || MODULES[1]);
    const currentSubTabs = computed(() => {
      const mod = currentModule.value;
      return mod.subTabs.map(id => {
        const labels = { cultivation:'修炼', techniques:'功法', skills:'技能', beast:'灵宠', map:'地图', dungeon:'副本', trial:'试炼', duel:'斗法', league:'联赛', sectwar:'宗门战', chat:'聊天', sect:'宗门', alliance:'仙盟', mail:'邮件', bailian:'百炼', inventory:'背包', equipment:'装备', exchange:'坊市', cave:'洞府', disciple:'传人', shijieshu:'世界书', settings:'设置', dictionary:'词典', announcement:'公告' };
    ...
    function switchModule(modId) {
      ...
      if (mod && mod.subTabs.length) {
        switchTab(mod.subTabs[0]);
      }
    }
    function switchTab(tabId) {
      ...
      for (const mod of MODULES) {
        if (mod.subTabs.includes(tabId)) {
          activeModule.value = mod.id;
          break;
        }
      }
      ensureGameDataForTab(tabId);
      api.wsSetBattleDetail(tabId === 'map');
    }
    // 旧兼容：保持tabs计算属性（部分老代码可能引用）
    const tabs = computed(() => {
      const labels = { announcement:'公告', chat:'聊天', character:'角色', cultivation:'修行', inventory:'背包', equipment:'装备', skills:'技能', techniques:'功法', map:'地图', bailian:'百炼', shijieshu:'世界书', cave:'洞府', disciple:'传人', sect:'宗门', alliance:'仙盟', exchange:'坊市', dungeon:'副本', duel:'斗法', league:'联赛', beast:'灵宠', sectwar:'宗门战争', trial:'试炼', mail:'邮件', dictionary:'词典', settings:'设置' };
    ...
    const mapListBySubTab = computed(() => {
      ...
      const realmMap = {
        renjie: 'first_heaven',
        second: 'second_heaven',
        third: 'third_heaven',
        fourth: 'fourth_heaven',
        fifth: 'fifth_heaven',
        sixth: 'sixth_heaven',
        seventh: 'seventh_heaven',
        eighth: 'eighth_heaven',
        ninth: 'ninth_heaven'
      };
      const targetRealm = realmMap[mapSubTab.value];
      if (targetRealm) {
        return maps.filter(m => {
          if (isNightmareMapData(m) || isLingjieMapData(m)) return false;
          const r = String(m.realm || '');
          // 第一重天：realm为空或'first_heaven'，且ID<100
          if (targetRealm === 'first_heaven') {
            return (!r || r === 'first_heaven') && (m.id < 100 || m.id >= 2000);
          }
          return r === targetRealm;
        });
      }
      // 默认：第一重天
    ...
    const tabsNeedGameData = new Set(['map', 'inventory', 'equipment', 'skills', 'techniques', 'bailian', 'dungeon', 'duel', 'league', 'trial', 'dictionary', 'cultivation']);
    ...
    /** 突破条件说明（用于 tooltip）——统一数据驱动版本 */
    function breakthroughConditionTooltip() {
      const lv = Math.floor(Number(player.level) || 1);
      // 突破节点配置：[atLevel, toLevel, realmName, baseRate, pillKey]
      const nodes = [
        [120, 121, '筑基', 0.20, 'foundation'],
        [136, 137, '结丹', 0.16, 'yunling'],
        [152, 153, '元婴', 0.14, 'yuanying'],
        [168, 169, '化神', 0.12, 'huashen'],
        [184, 185, '炼虚', 0.10, 'lianxu'],
        [200, 201, '合体', 0.08, 'heti'],
        [216, 217, '大乘', 0.06, 'dacheng'],
        [232, 233, '真仙', 0.05, 'zhenxian'],
        [248, 249, '金仙', 0.04, 'jinxian'],
        [264, 265, '太乙', 0.03, 'taiyi'],
        [280, 281, '大罗', 0.02, 'daluo'],
        [296, 297, '道祖', 0.01, 'daozu'],
      ];
      // 丹药配置：[pillKey, storedField, cap, bonusPerPill, label]
      const pillCfg = {
        foundation: ['breakthrough_foundation_pills_stored', 5, 0.20, '筑基丹'],
        yunling:    ['breakthrough_yunling_stored',         4, 0.20, '蕴灵丹'],
        yuanying:   ['breakthrough_yuanying_stored',        3, 0.20, '元婴育神丹'],
        huashen:    ['breakthrough_huashen_stored',         3, 0.20, '化神淬体丹'],
        lianxu:     ['breakthrough_lianxu_stored',          3, 0.20, '炼虚合道丹'],
        heti:       ['breakthrough_heti_stored',            3, 0.20, '合体归元丹'],
        dacheng:    ['breakthrough_dacheng_stored',         2, 0.25, '大乘渡劫丹'],
        zhenxian:   ['breakthrough_zhenxian_stored',        2, 0.25, '飞升接引丹'],
        jinxian:    ['breakthrough_jinxian_stored',         2, 0.25, '金仙证道丹'],
        taiyi:      ['breakthrough_taiyi_stored',           1, 0.30, '太乙归元丹'],
        daluo:      ['breakthrough_daluo_stored',           1, 0.30, '大罗混元丹'],
        daozu:      ['breakthrough_daozu_stored',           1, 0.30, '道祖证道丹'],
      };
      const node = nodes.find(n => n[0] === lv);
      if (!node) {
        const avail = nodes.map(n => n[0]).join('/');
        return `当前等级无法突破（需${avail}级）。`;
      }
      const [atLv, , realm, base, pillKey] = node;
      let rate = base;
      const parts = [`基础${Math.round(base*100)}%`];
      // 丹药加成
      const pc = pillCfg[pillKey];
      if (pc) {
        const [field, cap, perPill, label] = pc;
        const stored = Math.min(cap, Number(player[field]) || 0);
        const pillBonus = stored * perPill;
        rate += pillBonus;
        parts.push(`${label}${stored}枚×${Math.round(perPill*100)}%(+${Math.round(pillBonus*100)}%)`);
      }
      // 灵根品质加成
      const qualityBonusMap = { tian: 0.15, variant: 0.20, zhen: 0.05, wei: -0.05 };
      const sq = String(player.spirit_root_quality || 'zhen');
      const sqBonus = qualityBonusMap[sq] || 0;
      rate += sqBonus;
      const sqNames = { tian: '天灵根(+15%)', variant: '变异灵根(+20%)', zhen: '真灵根(+5%)', wei: '伪灵根(-5%)' };
      parts.push(sqNames[sq] || '灵根');
      // 六维基础加成
      ...
      const sixThreshold = atLv * 12;
      const sixBonus = sixSum > sixThreshold ? Math.min(0.25, (sixSum - sixThreshold) / (sixThreshold * 3)) : 0;
      rate += sixBonus;
      if (sixBonus > 0) parts.push(`六维${sixSum}(+${Math.round(sixBonus*100)}%)`);
      else parts.push(`六维${sixSum}(需>${sixThreshold})`);
      rate = Math.min(rate, 1);
      // 特殊说明
      let extra = '';
      if (pillKey === 'yuanying') extra = '\n元婴育神丹：击杀等级>自身的怪物可累积击杀次数，每3次+1%，上限45%。';
      if (pillKey === 'huashen') extra = '\n化神淬体丹：挑战秘境可累积次数，每次+3%，上限30%；心魔试炼通过+20%。';
      return `${realm}突破（当前概率: ${Math.round(rate*100)}%）\n` + parts.join(' + ') + `。\n服用对应丹药可大幅提升成功率（丹药在突破时自动消耗）。` + extra;
    }
    ...
    async function doLevelMax() {
      if (!confirm('一键升级将直接升到最高级（突破时自动停止）\n确定继续？')) return;
      try {
        const r = await api.levelMax();
        if (!r.ok) { showToast(r.error); return; }
        applyPlayer(r.player);
        showToast(r.msg || '升级完成');
      } catch (e) { showToast(e.message); }
    }
    ...
    const _ALCHEMY_LV_EXP = [0, 0, 100, 300, 600, 1000, 1600, 2500, 4000, 6000];
    const alchemyExpPercent = computed(() => {
      const lv = Number(player.alchemy?.level) || 1;
      const exp = Number(player.alchemy?.exp) || 0;
      const cur = _ALCHEMY_LV_EXP[lv] || 0;
      const next = _ALCHEMY_LV_EXP[Math.min(lv + 1, 9)] || _ALCHEMY_LV_EXP[9];
      if (next <= cur) return 100;
      return Math.min(100, Math.round(((exp - cur) / (next - cur)) * 100));
    });
    const alchemyNextLevelExp = computed(() => {
      const lv = Number(player.alchemy?.level) || 1;
      return _ALCHEMY_LV_EXP[Math.min(lv + 1, 9)] || _ALCHEMY_LV_EXP[9];
    });
    const alchemySuccessRate = (grade) => {
      const lv = Number(player.alchemy?.level) || 1;
      const diff = lv - (grade || 0);
      if (diff >= 0) return 100;
      if (diff === -1) return 70;
      if (diff === -2) return 40;
      return 15;
    };
    const alchemyGradeName = (g) => ['','一品','二品','三品','四品','五品','六品','七品','八品','九品'][g] || g+'品';
    ...
    // shijieshu (世界书) - world setting display
    const bailianLayer = ref(1); // 当前选中的重天 (1-9)
    const bailianLayers = [
      { id:1, name:'第一重天', title:'九州·炼气修仙', system:'古典九境流', desc:'近乎无限的大陆与海洋，灵气稀薄但人口最多。修士以炼气之道攀升，渡飞升雷劫方可进入第二重天。', geography:'东域平原丘陵，西域沙漠戈壁，南域原始森林沼泽，北域冰原雪域，中域修仙界最繁华之地。', forces:'正道三宗（太虚剑派、青云丹宗、天音阁）、魔教五派（血煞魔宗、万蛊门、尸傀宗、黑天魔教、焚天魔宫）、两大联盟（四海散修盟、万界商会）', realms:'炼气→筑基→金丹→元婴→化神→炼虚→合体→大乘→渡劫', secret:'灵根是上界投下的道种，修仙即被种田。飞升实则是被收割。', color:'#4a9eff' },
      { id:2, name:'第二重天', title:'蛊域·炼蛊流', system:'蛊真人式炼蛊', desc:'原始丛林、沼泽、瘴气之地，毒虫蛊物横行。修士以本命蛊为核心，养蛊、升蛊、争夺蛊道。', geography:'十万大山、瘴气沼泽、蛊虫巢穴、毒雾森林。', forces:'血煞魔宗分支、万蛊门本部、蛊仙世家', realms:'蛊徒→蛊师→蛊王→蛊皇→蛊仙', secret:'本命蛊可能是道种的另一种形态，蛊修本质仍是上界的养料。', color:'#8b5cf6' },
      { id:3, name:'第三重天', title:'蛮荒·炼体流', system:'肉身炼体', desc:'重力极大的蛮荒大陆，多火山、荒漠、裂谷。修士放弃法术，专注淬炼肉身，追求滴血重生。', geography:'火山荒漠、重力裂谷、蛮荒丛林、古神遗迹。', forces:'炼体宗门、蛮族部落、古神信徒', realms:'铜皮→铁骨→金身→不灭→圣体→神魔体→混沌体', secret:'炼体者的神魔体精血是第二重天顶级蛊虫的饵料，死后肉身被上层炼成神像。', color:'#ef4444' },
      { id:4, name:'第四重天', title:'星海·神道流', system:'神道香火', desc:'无尽星海，漂浮着无数神国。修士不修自身，修众生信仰，开辟神国、凝聚神格。', geography:'无尽星海、神国浮岛、信仰星河、神火熔炉。', forces:'诸神联盟、星君神国、万界商会上层', realms:'游神→正神→星君→帝君→天帝→神主', secret:'诸神暗中在下界建立教会，将下界众生转化为信徒，收割信仰之力。', color:'#f59e0b' },
      { id:5, name:'第五重天', title:'符环·符箓流', system:'符箓阵道', desc:'由无数符文浮岛构成的星环，岛上铭刻着上古法则。修士以符为笔、以阵为界，铭刻天地法则。', geography:'符文浮岛星环、法则碎片海洋、古神符文遗迹。', forces:'天机阁总部、符道宗门、法则猎人', realms:'符徒→符师→符宗→符王→符皇→符圣→符帝→符仙', secret:'符道修士是九重天的工匠与规则书写者，被上层雇佣布置因果法阵。', color:'#10b981' },
      { id:6, name:'第六重天', title:'冥河·轮回流', system:'轮回因果', desc:'轮回之海与冥河，灵魂在此流转、转世。修士掌控因果线，可斩断他人因果、操控转世轮回。', geography:'轮回之海、冥河、因果蛛网、转世台。', forces:'轮回殿、因果宗、命运编织者', realms:'入轮回→断因果→掌轮回→超脱轮回', secret:'第六重天是九重天的命运管理者，暗中编织下界的命运轨迹。', color:'#6366f1' },
      { id:7, name:'第七重天', title:'时空·大道流', system:'时空大道', desc:'时空乱流，有碎片大陆与时间漩涡。修士修行时间与空间法则，可加速时间、凝固空间。', geography:'时空乱流、碎片大陆、时间漩涡、空间裂隙。', forces:'时空守护者、裂隙行者、时间领主', realms:'时空行者→时空领主→时空之王→时空主宰', secret:'第七重天负责维持九重天之间的时空壁垒，防止下界力量倒灌。', color:'#0ea5e9' },
      { id:8, name:'第八重天', title:'混沌·本源流', system:'本源混沌', desc:'混沌虚空，空间极小，混沌之气浓郁到近乎实质。修士感悟世界本源，吞噬万物化为混沌。', geography:'混沌虚空、本源裂缝、归墟之眼、太初之门。', forces:'混沌修士、本源守卫、归墟修士', realms:'归元→混沌→太初→无极', secret:'第八重天是九重天的心脏，直接抽取下方七重天的本源作为燃料。', color:'#78350f' },
      { id:9, name:'第九重天', title:'仙宫·超脱流', system:'仙之法则', desc:'仙宫琼楼，悬浮于最高处。此层没有固定修炼体系，只有仙的法则。修士需将前八重天所有道途融会贯通。', geography:'仙宫琼楼、仙气缭绕、道劫雷海、超脱之门。', forces:'仙尊、超脱者、分神转世者', realms:'真仙→天仙→金仙→大罗金仙→混元大罗→超脱', secret:'仙位有限，需杀伐争夺。超脱者可分化亿万分身，转世重修万千小世界。', color:'#dc2626' }
    ];
    ...
    return {
      ...
      createForm, rollCreateRoots, doCreate,
      activeTab, tabs, mapSubTab, mapListBySubTab,
      // 新模块化导航
      MODULES, activeModule, currentModule, currentSubTabs, switchModule, switchTab,
      charRealm, charStage, breakthroughConditionTooltip, doLevelUp, doLevelMax, doBreakthrough,
      ...
      baiyiTab, bailianLayer, bailianLayers,
      alchemyVisibleRecipes, alchemyExpPercent, alchemyNextLevelExp, alchemySuccessRate, alchemyGradeName, _alchemyGradeBg, _alchemyGradeColor, _alchemyGradeColorDark,
      buildingCurrentDesc, buildingNextDesc,
      sect, alch, exch, alli, chat, dung, duel, league, beast, sectwar, sett, ai,
      cult,
      ...
      DICTIONARY_ENTRIES, dictCategory, dictPage, dictPageCount, dictPageEntries, dictFiltered, DICT_PAGE_SIZE, SPIRIT_ROOT_QUALITY_LABELS,
