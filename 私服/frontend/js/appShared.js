// 《仙 九重天阙》十三大境界 · 人界/灵界/仙界三层分割（等级封顶 320）
// 炼气 1-120（13 层）→ 筑基 121-136 → 结丹 137-152 → 元婴 153-168 → 化神 169-184
// → 炼虚 185-200 → 合体 201-216 → 大乘 217-232 → 真仙 233-248 → 金仙 249-264
// → 太乙 265-280 → 大罗 281-296 → 道祖 297-320
export const REALMS = [
  { min: 1, max: 120, name: '炼气' }, { min: 121, max: 136, name: '筑基' },
  { min: 137, max: 152, name: '结丹' }, { min: 153, max: 168, name: '元婴' },
  { min: 169, max: 184, name: '化神' }, { min: 185, max: 200, name: '炼虚' },
  { min: 201, max: 216, name: '合体' }, { min: 217, max: 232, name: '大乘' },
  { min: 233, max: 248, name: '真仙' }, { min: 249, max: 264, name: '金仙' },
  { min: 265, max: 280, name: '太乙' }, { min: 281, max: 296, name: '大罗' },
  { min: 297, max: 320, name: '道祖' },
];

const HUMAN_REALMS = ['炼气', '筑基', '结丹', '元婴', '化神'];
const SPIRIT_REALMS = ['炼虚', '合体', '大乘'];

/** 三层分割：人界 / 灵界 / 仙界 */
export function getRealmGroup(lv) {
  const r = getRealm(lv);
  if (HUMAN_REALMS.includes(r)) return '人界';
  if (SPIRIT_REALMS.includes(r)) return '灵界';
  return '仙界';
}
...
export function getRealmStage(lv) {
  ...
  if (realm === '炼气' && lv >= 1 && lv <= 120) {
    const layer = Math.min(13, Math.max(1, Math.ceil((lv * 13) / 120)));
    return layer + '层';
  }
...
// ── 灵根随机生成（与后端 player.rollSpiritRoots 同规则） ──────────────────
...
export function rollSpiritRoots() {
  const r = Math.random();
  let quality;
  let count;
...
// 装备品阶颜色（法器→至宝）
export const QUALITY_COLORS = { 1: '#aaa', 2: '#4a4', 3: '#48f', 4: '#a4f', 5: '#fa4', 6: '#f44', 7: '#f4a', 8: '#ff0' };
...
// 装备品阶名称（九重天体系）
const EQUIP_QUALITY_NAMES = { 1: '凡器', 2: '法器', 3: '灵器', 4: '灵宝', 5: '法宝', 6: '古宝', 7: '仙器', 8: '至宝' };

export function qualityName(q) {
  return EQUIP_QUALITY_NAMES[q] || String(q);
}

// 炼丹品阶名称
const ALCHEMY_GRADE_NAMES = { 0: '凡品', 1: '一品', 2: '二品', 3: '三品', 4: '四品', 5: '五品', 6: '六品', 7: '七品', 8: '八品', 9: '九品' };

export function alchemyGradeName(g) {
  return ALCHEMY_GRADE_NAMES[g] || String(g) + '品';
}
