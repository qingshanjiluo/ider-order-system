...
export function useChat() {
  ...
  let since = 0;
  let timer = null;
  let pollActive = true;          // ����ҳ�ɼ�������ӦƵ�ʣ�
  let initialized = false;
  let useWebSocket = false;       // �Ƿ�ʹ�� WebSocket ģʽ
  ...
  const loadMsg = async () => {
    ...
    if (r?.ok && r.messages?.length) {
      // ȥ�� 1�������������ʵ��Ϣ�滻�����ֹۻ��ԣ����Ȱ���˷��ص���Ϣ id ��ȷƥ�䣬
      // ����ǰ���ʱ��ƫ��� ts �Բ��ϣ�
      messages.value = messages.value.filter(m => {
        if (!m.self) return true;
        if (m._sentId) return !r.messages.some(rm => rm.id === m._sentId);
        const mts = Number(m.ts) || 0;
        return !r.messages.some(rm => rm.text === m.text && Math.abs(Number(rm.ts || 0) - mts) <= 2);
      });
      // ȥ�� 2��ȫ�ְ���Ϣ id ȥ�أ���ֹ������ȡ��send ���ֶ� loadMsg ����ѯ��̬������ظ�
      const seen = new Set(messages.value.filter(m => !m.self).map(m => m.id));
      const fresh = r.messages.filter(m => !seen.has(m.id));
      const before = messages.value.length;
      messages.value.push(...fresh.map(m => ({ ...m, self: (m.account_id === (_player.account_id || 0)) })));
      if (messages.value.length > 200) messages.value.splice(0, messages.value.length - 200);
      since = Math.max(since, ...fresh.map(m => m.id || m.timestamp || 0));
      const newMsgs = messages.value.length - before;
      if (newMsgs > 0) {
        if (!pollActive) unread.value += newMsgs;
  ...
  const handleWsMessage = (data) => {
    if (data.type === 'message' && data.channel === channel.value) {
      // ����Ƿ����Լ�����Ϣ�������ظ���
      const isSelf = data.username === (_player.name || _player.username);
      if (!isSelf) {
        const exists = messages.value.some(m => m.id === `ws-${data.timestamp}-${data.username}`);
        if (!exists) {
          messages.value.push({
            id: `ws-${data.timestamp}-${data.username}`,
          ...
          if (messages.value.length > 200) messages.value.splice(0, messages.value.length - 200);
          if (!pollActive) unread.value += 1;
          scrollToBottomIfNear();
        }
      }
    } else if (data.type === 'presence') {
      // ����״̬���£���ѡ��ʾ��
      console.log(`[chat] ${data.action}: ${data.username}, online: ${data.online}`);
    }
  };
  ...
  const send = async () => {
    ...
    try {
      // ����ͨ�� WebSocket ���ͣ��������������
      if (useWebSocket && api.isWsConnected()) {
        // WebSocket ���ͺ���Ϣ��ͨ���㲥����������Ҫ�ֹۻ���
        // ��Ϊ��һ���ԣ���Ȼ���ֹۻ���
        const mine = {
          id: `local-${Date.now()}`,
          username: _player.name || _player.username || '��',
          text, ts: Math.floor(Date.now() / 1000), self: true,
        };
        messages.value.push(mine);
        if (messages.value.length > 200) messages.value.splice(0, messages.value.length - 200);
        scrollToBottomIfNear();
        // ͨ�� HTTP API ���ͣ�WebSocket �㲥�ɷ�����������
        const r = await safe(() => api.chatSend(channel.value, text));
        if (r?.ok) {
          input.value = '';
          // �Ƴ��ֹۻ��ԣ���������㲥������
          messages.value = messages.value.filter(m => m.id !== mine.id);
        } else if (r) {
          messages.value = messages.value.filter(m => m.id !== mine.id);
          _showToast(r.error || '����ʧ��');
        }
      } else {
        // HTTP ��ѯģʽ
        const r = await safe(() => api.chatSend(channel.value, text));
        if (r?.ok) {
          ...
          if (messages.value.length > 200) messages.value.splice(0, messages.value.length - 200);
          scrollToBottomIfNear();
        } else if (r) _showToast(r.error || '����ʧ��');
      }
  ...
  const startPoll = () => {
    ...
    // ���� WebSocket ���ӣ�ʵʱ���죩
    const username = _player.name || _player.username || '\u533f\u540d';
    api.onWsMessage(handleWsMessage);
    api.connectWs(channel.value, username);

    // �ȴ� WebSocket ���ӽ����2 �볬ʱ��
    setTimeout(() => {
      useWebSocket = api.isWsConnected();
      if (useWebSocket) {
        console.log(`[chat] WebSocket connected for channel: ${channel.value}`);
      } else {
        console.log(`[chat] WebSocket unavailable, using HTTP polling`);
      }
    }, 2000);

    // HTTP ��ѯ��Ϊ���ݣ���ʹ WebSocket ����Ҳ������Ƶ��ѯ�Է���©��
    const tick = () => {
      if (!initialized) { timer = null; return; }
      // WebSocket ģʽ�½�����ѯƵ�ʣ�10 ��һ����Ϊ���ݣ�
      if (!useWebSocket || !api.isWsConnected()) {
        loadMsg();
      }
      const v = document.visibilityState !== 'hidden';
      const interval = useWebSocket && api.isWsConnected()
        ? 15000
        : ((v && pollActive) ? 10000 : 30000);
  ...
  const switchChannel = (ch) => {
    channel.value = ch; messages.value = []; since = 0; unread.value = 0; pollActive = true; loadMsg();
  };
  const setPollActive = (val) => {
    pollActive = !!val;
    if (pollActive) { unread.value = 0; loadMsg(); }
  };

  return { channel, messages, input, unread, isTyping, fmtTime, send, startPoll, stopPoll, switchChannel, markRead, setPollActive };
}
...
export function useDuel() {
  ...
  const loadLogs = async (page) => {
    ...
    const r = await safe(() => api.cityDuelLogs(p, 20));
    if (r?.ok) {
      ...
      logPage.value = r.page || p;
      logTotalPages.value = r.total_pages || 1;
    }
  };
  ...
  const loadArena = async () => {
    ...
    const r = await safe(() => api.arenaInfo());
    arenaLoading.value = false;
    if (r?.ok) arenaInfo.value = r;
  };

  const registerArena = async () => {
    const r = await safe(() => api.arenaRegister());
    if (r?.ok) {
      _showToast('报名成功，组别：' + (r.group_name || ''));
      await loadArena();
    } else {
      _showToast(r?.error || '报名失败');
    }
  };

  const startArenaMatch = async (bracketRound, bracketMatchIndex) => {
    const r = await safe(() => api.arenaStart(bracketRound, bracketMatchIndex));
    if (r?.ok && r.match) {
      const duelStartResult = await safe(() => api.cityDuelStart(r.opponent.account_id));
      if (duelStartResult?.ok) {
        duelBattleId.value = duelStartResult.battle_id;
        duelState.value = duelStartResult.state;
        duelResult.value = null;
        duelLog.value = [];
        duelRunning.value = true;
        _pushDuelLog(`擂台对战${r.opponent.name} Lv.${r.opponent.level}`);
        autoAdvance();
      } else {
        _showToast(duelStartResult?.error || '发起对战失败');
      }
    } else {
      _showToast(r?.error || '匹配失败');
    }
  };

  return {
    targets,
    logs, logPage, logTotalPages,
    rank,
    duelSubTab,
    duelBattleId,
    arenaInfo,
...
export function useBeast() {
  ...
  const loading = ref(false);
  const feedItemPickerOpen = ref(false);
  const feedTargetBeastId = ref(0);
  ...
  const loadRoster = async () => {
    loading.value = true;
    const r = await safe(() => api.beast.roster());
    loading.value = false;
    if (r?.ok) {
      roster.value = r.roster || [];
      activeId.value = r.active_id || 0;
    }
  };

  const deploy = async (beastId) => {
    if (hr(await safe(() => api.beast.deploy(beastId)), beastId > 0 ? '出战成功' : '已收回灵宠')) {
      await loadRoster();
      if (_player && _player.beasts) _player.beasts.active_id = beastId;
    }
  };

  const openFeed = (beastId) => {
    feedTargetBeastId.value = beastId;
    feedItemPickerOpen.value = true;
  };

  const feed = async (itemId) => {
    feedItemPickerOpen.value = false;
    if (hr(await safe(() => api.beast.feed(feedTargetBeastId.value, itemId)), '喂养成功')) {
      await loadRoster();
    }
  };

  const feedWithCount = async () => {
    feedItemPickerOpen.value = false;
    const count = Math.min(100, Math.max(1, feedCount.value || 1));
    if (hr(await safe(() => api.beast.feed(feedTargetBeastId.value, 0, count)), '喂养成功')) {
      await loadRoster();
    }
  };

  const evolve = async (beastId) => {
    if (!confirm('确定进化此灵宠？')) return;
    if (hr(await safe(() => api.beast.evolve(beastId)), '进化成功')) {
      await loadRoster();
    }
  };

  const release = async (beastId) => {
    if (!confirm('确定放生此灵宠？放生后无法恢复！')) return;
    if (hr(await safe(() => api.beast.release(beastId)), '已放生')) {
      await loadRoster();
    }
  };

  const activeBeast = computed(() => roster.value.find(b => b.id === activeId.value) || null);

  const filteredRoster = computed(() => {
    let list = [...roster.value];
    if (filterElement.value !== 'all') {
      list = list.filter(b => (b.template_element || b.element) === filterElement.value);
    }
    list.sort((a, b) => {
      if (sortBy.value === 'level') return (b.level || 1) - (a.level || 1);
      if (sortBy.value === 'tier') return (b.template_tier || b.tier || 1) - (a.template_tier || a.tier || 1);
      if (sortBy.value === 'attack') return ((b.stats?.attack || b.attack) || 0) - ((a.stats?.attack || a.attack) || 0);
      if (sortBy.value === 'hp') return ((b.stats?.hp || b.hp) || 0) - ((a.stats?.hp || a.hp) || 0);
      return 0;
    });
    return list;
  });
  ...
  return {
    roster, activeId, loading, activeBeast, filteredRoster,
    feedItemPickerOpen, feedTargetBeastId,
    sortBy, filterElement, feedCount,
    loadRoster, deploy, openFeed, feed, feedWithCount, evolve, release,
  };
}

export function useSectWar() {
  const territories = ref([]);
  const battles = ref([]);
  const seasonId = ref(0);
  const warEnabled = ref(true);
  const loading = ref(false);
  ...
  const loadTerritories = async () => {
    loading.value = true;
    const r = await safe(() => api.sectWar.territories());
    loading.value = false;
    if (r?.ok) {
      territories.value = r.territories || [];
      seasonId.value = r.season_id || 0;
      warEnabled.value = r.war_enabled !== false;
    }
  };

  const loadBattles = async () => {
    const r = await safe(() => api.sectWar.battles());
    if (r?.ok) {
      battles.value = r.battles || [];
      startCountdowns();
    }
  };
  ...
  const startCountdowns = () => {
    ...
    const update = () => {
      ...
      for (const b of battles.value) {
        if (b.status === 'active' && b.ends_at) {
          const left = b.ends_at - now;
          countdowns[b.id] = left > 0 ? formatCountdown(left) : '已结束';
        }
      }
    };
    update();
  ...
  const declare = async (territoryId) => {
    if (!confirm('确定对该领地宣战？战争将持续1小时。')) return;
    if (hr(await safe(() => api.sectWar.declare(territoryId)), '宣战成功！')) {
      await loadBattles();
    }
  };

  const defend = async (battleId) => {
    if (hr(await safe(() => api.sectWar.defend(battleId)), '已加入防御')) {
      await loadBattles();
    }
  };

  const mySectId = computed(() => _player?.sect_id || 0);

  const elementIcon = (el) => {
    const icons = { fire: '🔥', water: '💧', wood: '🌿', metal: '⚔️', earth: '🪨', dark: '🌑', none: '⭐' };
    return icons[el] || '⭐';
  };

  const elementColor = (el) => {
    const colors = { fire: '#e74c3c', water: '#3498db', wood: '#2ecc71', metal: '#f39c12', earth: '#8b4513', dark: '#6c3483', none: '#9b59b6' };
    return colors[el] || '#9b59b6';
  };

  return {
    territories, battles, seasonId, warEnabled, loading, mySectId, countdowns,
    loadTerritories, loadBattles, declare, defend,
    elementIcon, elementColor,
  };
}

export function useAI() {
  const aiPlayers = ref([]);
  const aiLeaderboard = ref([]);
  ...
  const loadAIPlayers = async () => {
    loading.value = true;
    const r = await safe(() => api.ai.players());
    loading.value = false;
    if (r?.ok) {
      aiPlayers.value = r.players || [];
    }
  };

  const loadLeaderboard = async () => {
    const r = await safe(() => api.ai.leaderboard());
    if (r?.ok) {
      aiLeaderboard.value = r.leaderboard || [];
    }
  };

  const loadStatus = async () => {
    const r = await safe(() => api.ai.status());
    if (r?.ok) {
      aiStatus.value = r;
    }
  };

  const selectAI = async (accountId) => {
    const r = await safe(() => api.ai.player(accountId));
    if (r?.ok) {
      selectedAI.value = r.player;
    }
  };

  const interact = async (targetId, action) => {
    return await safe(() => api.ai.interact(targetId, action));
  };

  const initAI = async () => {
    loading.value = true;
    const r = await safe(() => api.ai.init());
    loading.value = false;
    return r;
  };

  const personalityName = (p) => {
    const names = {
      AGGRESSIVE: '\u597d\u6597', DEFENSIVE: '\u7a33\u5065', SOCIAL: '\u793e\u4ea4', ECONOMIC: '\u5546\u4eba',
      CULTIVATOR: '\u82e6\u4fee', BALANCED: '\u5168\u80fd', NEWBIE: '\u840c\u65b0', VETERAN: '\u5927\u4f6c',
    };
    return names[p] || p;
  };

  const qualityNameAI = (q) => {
    const names = { variant: '\u53d8\u5f02', tian: '\u5929\u7075\u6839', zhen: '\u771f\u7075\u6839', wei: '\u4f2a\u7075\u6839' };
    return names[q] || q;
  };

  return {
    aiPlayers, aiLeaderboard, aiStatus, loading, selectedAI,
    loadAIPlayers, loadLeaderboard, loadStatus, selectAI, interact, initAI,
    personalityName, qualityName: qualityNameAI,
  };
}
...
export function useAdmin() {
  const loading = ref(false);
  const activeSubTab = ref('players');

  // 玩家管理
  const players = ref([]);
  const playersTotal = ref(0);
  const playersPage = ref(1);
  const playersSearch = ref('');
  ...
  const loadPlayers = async (page = 1) => {
    ...
    if (playersSearch.value) params.search = playersSearch.value;
    const r = await safe(() => api.admin.players(params));
    loading.value = false;
    if (r?.ok) {
      players.value = r.players || [];
      playersTotal.value = r.total || 0;
      playersPage.value = r.page || 1;
    }
  };

  const loadPlayerDetail = async (accountId) => {
    const r = await safe(() => api.admin.playerDetail(accountId));
    if (r?.ok) selectedPlayer.value = r.player;
  };

  const banPlayer = async (accountId, reason, expiresAt) => {
    return await safe(() => api.admin.banPlayer(accountId, reason, expiresAt));
  };

  const unbanPlayer = async (accountId) => {
    return await safe(() => api.admin.unbanPlayer(accountId));
  };

  const setAdmin = async (accountId, isAdmin) => {
    return await safe(() => api.admin.setAdmin(accountId, isAdmin));
  };

  const setStats = async (accountId, stats) => {
    return await safe(() => api.admin.setStats(accountId, stats));
  };

  const sendMail = async (accountId, subject, content, items) => {
    return await safe(() => api.admin.sendMail(accountId, subject, content, items));
  };

  const loadAIPlayers = async () => {
    loading.value = true;
    const r = await safe(() => api.admin.aiPlayers());
    loading.value = false;
    if (r?.ok) aiPlayers.value = r.aiPlayers || [];
  };

  const loadAILeaderboard = async () => {
    const r = await safe(() => api.admin.aiLeaderboard());
    if (r?.ok) aiLeaderboard.value = r.leaderboard || [];
  };

  const aiTick = async (accountId) => {
    loading.value = true;
    const r = await safe(() => api.admin.aiTick(accountId));
    loading.value = false;
    return r;
  };
  ...
  const aiInit = async () => {
    loading.value = true;
    const r = await safe(() => api.admin.aiInit());
    loading.value = false;
    return r;
  };
  ...
  const aiReset = async () => {
    loading.value = true;
    const r = await safe(() => api.admin.aiReset());
    loading.value = false;
    return r;
  };

  const loadStats = async () => {
    const r = await safe(() => api.admin.systemStats());
  ...
  const loadAccounts = async (page = 1) => {
    ...
    const params = { page, pageSize: 20 };
    if (playersSearch.value) params.search = playersSearch.value;
    const r = await safe(() => api.admin.accounts(params));
    loading.value = false;
    if (r?.ok) {
      accounts.value = r.accounts || [];
      accountsTotal.value = r.total || 0;
      accountsPage.value = r.page || 1;
    }
  };
  ...
  return {
    ...
    // 玩家
    players, playersTotal, playersPage, playersSearch, selectedPlayer,
    loadPlayers, loadPlayerDetail, banPlayer, unbanPlayer, setAdmin, setStats, sendMail,
    // AI
    aiPlayers, aiLeaderboard, loadAIPlayers, loadAILeaderboard, aiTick, aiInit, aiReset,
    // 系统
    stats, accounts, accountsTotal, accountsPage, loadStats, loadAccounts,
    personalityName,
  };
}
