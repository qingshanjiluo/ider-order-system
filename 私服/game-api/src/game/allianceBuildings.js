/**
 * 仙盟建筑逻辑（ESM 版）
 * 迁移自 server/game/allianceBuildings.js。
 */
import { getItems } from './dataLoader.js';

export const BUILDING_NAMES = { statue: '雕像', spirit_pool: '灵池', garden: '仙园', enlightenment_tree: '悟道树', treasury: '宝阁', gate: '仙门' };
export const BUILDING_KEYS = ['statue', 'spirit_pool', 'garden', 'enlightenment_tree', 'treasury', 'gate'];
...
export function getUpgradeCost(currentLevel, building) {
...
export function getMemberLimit(gateLevel) {
  const lv = Math.max(1, Math.min(10, intVal(gateLevel, 1)));
  return 50 + (lv - 1) * 30;
}

/** 灵池：属性加成 1%~5% */
export function getSpiritPoolBonusPct(level) {
  const lv = Math.max(1, Math.min(10, intVal(level, 1)));
  return 1 + (lv - 1) * 4 / 9;
}

/** 悟道树：经验加成 1%~4% */
export function getEnlightenmentBonusPct(level) {
  const lv = Math.max(1, Math.min(10, intVal(level, 1)));
  return 1 + (lv - 1) * 3 / 9;
}

/** 材料捐献价值：1阶3 2阶10 3阶20 4阶40 5阶80 */
export const DONATE_TIER_VALUE = { 1: 3, 2: 10, 3: 20, 4: 40, 5: 80 };
...
export function getMaterialTier(item) {
...
export function getDonateValue(item, count) {
 ...
 */
export const GARDEN_TIER_WEIGHTS = {
  1:  [85, 15,  0, 0],
  2:  [78, 22,  0, 0],
  3:  [72, 28,  0, 0],
  4:  [65, 35,  0, 0],
...
export function getGardenMaxTier(level) {
  const lv = Math.max(1, Math.min(10, intVal(level, 1)));
  if (lv >= 9) return 6;
  if (lv >= 5) return 5;
  return 4;
...
export function getGardenMaterialPool(level) {
  const lv = Math.max(1, Math.min(10, intVal(level, 1)));
...
export function pickGardenMaterial(level, rng01) {
