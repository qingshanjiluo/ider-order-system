/**
 * 百艺后台任务结算（Worker 版）
 * 迁移自 server/game/backgroundJobs.js，省略 cron 部分，仅保留路由同步调用的
 * settleBackgroundJobsForPlayer。原模块直接 require('../dbAsync')，
 * 本版本由调用方传入 db（createDb(env) 实例），其余逻辑与原版一致。
 */
...
export async function settleBackgroundJobsForPlayer(db, accountId, player, nowSec) {
  ...
  if (baiyiJob && intVal(baiyiJob.finish_at, 0) > 0 && intVal(baiyiJob.finish_at, 0) <= now) {
    ...
    let delivered = false;
      // 投递成功判定：mid>0 为新邮件；mid=-1 为去重命中（邮件已存在，视为已投递）；
      // mid===0 为插入失败 → 保留 pending_job 不清除，下次请求重试，避免产物永久丢失
      const deliveredOk = (v) => Number(v) !== 0;
      if (subType === 'forging' && result.equipment && typeof result.equipment === 'object' && Object.keys(result.equipment).length > 0) {
        ...
        if (deliveredOk(mid)) mailed += 1;
        delivered = deliveredOk(mid);
    } else {
