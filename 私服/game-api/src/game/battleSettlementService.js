import { getEnemyById, getMapById, getItemById, getItems, getDungeonEnemyById, getSkillById, getTechniqueById } from './dataLoader.js';
import * as ops from './playerOps.js';
import * as cave from './cave.js';
import { getBattleVictoryHealPercent, calcSpiritRootBonuses, calcAllPassiveEffects } from './combatUtils.js';
import { touchActivity } from './universalTime.js';
import { calculateRestTime } from './battleTiming.js';
import { isNightmareMap } from './battleEncounterFactory.js';
import { isLingjieMap, getLingjieRewardBonus } from './lingjie.js';

/** 原 settleKillTaskProgress（来自 routes/online/sectTaskRoutes，纯函数内联） */
function settleKillTaskProgress(player, enemyId) {
  const tasks = Array.isArray(player && player.sect_tasks) ? player.sect_tasks : [];
  for (let i = 0; i < tasks.length; i += 1) {
    const t = tasks[i] || {};
    if (String(t.type || '') !== 'kill_enemy') continue;
    if (!Boolean(t.accepted)) continue;
    if (intVal(t.target_id, 0) !== intVal(enemyId, 0)) continue;
    t.progress = intVal(t.progress, 0) + 1;
    tasks[i] = t;
  }
...
let _settlementDb = null;
export function setSettlementDb(db) {
  _settlementDb = db;
}
...
function settleDrops(player, enemy, dropBuff, enemySource = 'wild', mapInfo = null) {
  ...
  if (droppedCountRef.count < maxDrops && String(enemy.type || '') === 'beast') {
    const tier1Items = [400, 401, 402, 403, 404, 440, 441, 442, 443, 444];
    const tier2Items = [405, 406, 407, 408, 409, 445, 446, 447, 448, 449];
    const tier3Items = [410, 411, 412, 413, 414, 450, 451, 452, 453, 454];
    const tier4Items = [415, 416, 417, 418, 419, 455, 456, 457, 458, 459];
    const tier5Items = [420, 421, 422, 423, 424, 460, 461, 462, 463, 464];
...
async function finalizeBattle(session, state, outcomeLike) {
  ...
  if (victory && !speedHackDetected) {
    ...
    // 灵根品质修炼速度：天灵根/变异灵根更快，伪灵根更慢
    const rootExpMult = ops.getSpiritRootExpMult ? Math.max(0, Number(ops.getSpiritRootExpMult(player)) || 1) : 1;
    expGain = Math.floor(baseExp * (1.0 + expBuff) * enlightenmentMult * wildExpMult * mapExpMult * rootExpMult);
    const expMultiplier = Math.max(1e-9, (1.0 + expBuff) * enlightenmentMult * wildExpMult * mapExpMult * rootExpMult);
  ...
  // B5 离线基准记账：只累计野外胜利场的实际入账收益，供离线挂机结算参考
  if (victory && !speedHackDetected && source === 'wild') {
    try {
      if (!player.earn_stats || typeof player.earn_stats !== 'object') player.earn_stats = { exp_total: 0, spirit_total: 0, battle_count: 0, ts: 0 };
      player.earn_stats.exp_total = (Number(player.earn_stats.exp_total) || 0) + expGain;
      player.earn_stats.spirit_total = (Number(player.earn_stats.spirit_total) || 0) + spiritGain;
      player.earn_stats.battle_count = (Number(player.earn_stats.battle_count) || 0) + 1;
      player.earn_stats.ts = Math.floor(Date.now() / 1000);
    } catch {}
  }
...
export { finalizeBattle, _normalizeBattleOutcome as normalizeBattleOutcome };
