/**
 * 灵界环境系统（迁移自 server/game/lingjie.js，纯函数无 Node 依赖）
 */
...
export const LINGJIE_ENVIRONMENTS = {
  inverse_field: {
    id: 'inverse_field', name: '逆势天幕', effect_desc: '前3次行动内，双方直接伤害降低35%。', reward_desc: '怪物经验提高30%。',
    scaling: { hpRatio: 1.35, attackRatio: 0.92, spellRatio: 1.04, defenseRatio: 1.10, agilityRatio: 1.02, expRatio: 1.55 }
  },
  crack_armor: {
    id: 'crack_armor', name: '裂甲风蚀', effect_desc: '行动后若物防与法防合计过低，将受到基于生命上限的裂甲反噬。', reward_desc: '怪物经验提高20%。',
    scaling: { hpRatio: 1.55, attackRatio: 0.95, spellRatio: 0.95, defenseRatio: 1.85, agilityRatio: 0.96, expRatio: 1.65 }
  },
  overload_sea: {
    id: 'overload_sea', name: '过载雷海', effect_desc: '修士单段伤害过高会触发过载，后续两次行动伤害与速度下降。', reward_desc: '阵纹掉落概率变为三倍。',
    scaling: { hpRatio: 1.32, attackRatio: 1.02, spellRatio: 1.00, defenseRatio: 1.08, agilityRatio: 1.14, expRatio: 1.70 }
  },
  shield_break: {
    id: 'shield_break', name: '碎盾潮汐', effect_desc: '每次行动后叠加破势，按层数持续侵蚀生命并压低输出。', reward_desc: '怪物灵石提高35%。',
    scaling: { hpRatio: 1.42, attackRatio: 1.00, spellRatio: 1.06, defenseRatio: 1.18, agilityRatio: 1.00, expRatio: 1.60 }
  },
  attrition_field: {
    id: 'attrition_field', name: '衰荣旷野', effect_desc: '战斗开始即生效，行动者每次行动后承受15%最大生命的衰荣侵蚀。', reward_desc: '通用掉落概率提高25%。',
    scaling: { hpRatio: 1.60, attackRatio: 1.08, spellRatio: 1.02, defenseRatio: 1.24, agilityRatio: 0.98, expRatio: 1.72 }
  },
  mire_swamp: {
    id: 'mire_swamp', name: '迟滞泥沼', effect_desc: '每次行动后会被抽离法力并承受侵蚀伤害，低法力时惩罚加重。', reward_desc: '炼虚期六阶及以上材料掉率提高50%。',
    scaling: { hpRatio: 1.46, attackRatio: 0.98, spellRatio: 1.05, defenseRatio: 1.20, agilityRatio: 0.94, expRatio: 1.62 }
  }
};

export const LINGJIE_BASE_REWARD_BONUS = Object.freeze({ exp_mult: 1.15, drop_mult: 1.25, rune_drop_mult: 1, spirit_stone_mult: 1, tier6_drop_mult: 1 });

export const LINGJIE_DESIGN_LEVEL = 270;

export function getLingjieLevelScaleFactor(playerLevel) {
  const lv = Math.max(1, intVal(playerLevel, 1));
  if (lv <= LINGJIE_DESIGN_LEVEL) return clamp(lv / LINGJIE_DESIGN_LEVEL, 0.7, 1.0);
  const progress = (lv - LINGJIE_DESIGN_LEVEL) / 120;
  return clamp(1 + progress * 0.9 + progress * progress * 1.1, 1.0, 3.2);
}

export function isLingjieMap(map) {
  if (!map || typeof map !== 'object') return false;
  if (map.is_lingjie === true) return true;
  return String(map.realm || '').trim() === 'lingjie';
}

export function getLingjieEnvironment(map) {
...
export function getLingjieRewardBonus(map) {
  if (!isLingjieMap(map)) {
    return { exp_mult: 1, drop_mult: 1, rune_drop_mult: 1, spirit_stone_mult: 1, tier6_drop_mult: 1 };
  }
...
export function buildLingjieBattleContext(map) {
...
export function scaleLingjieEnemyForPlayer(enemy, map, player) {
  if (!isLingjieMap(map) || !enemy || typeof enemy !== 'object' || !player || typeof player !== 'object') return enemy;
  const env = getLingjieEnvironment(map);
