...
// 离线刷怪经验加成：相比在线更高的收益，体现挂机投入
const OFFLINE_EXP_MULT = 1.5;
...
function simulateOfflineBattles(player, offlineSeconds) {
  ...
  while (elapsedSec < offlineSec && (wins + losses + draws) < MAX_BATTLES_PER_SESSION) {
    ...
    if (victory) {
      ...
      const rootMult = ops.getSpiritRootExpMult ? ops.getSpiritRootExpMult(player) : 1;
      const baseExp = Math.floor(Number(enemy.exp) || 10);
      const exp = Math.floor(baseExp * (1.0 + expBuff) * enlightenmentMult * rootMult * OFFLINE_EXP_MULT);
      totalExp += exp;
