...
function levelUp(player) {
  ...
  if (lv >= 320) return { ok: false, error: '已达道祖大圆满，天下无敌' };
  if (BREAKTHROUGH_LEVELS.includes(lv)) return { ok: false, error: '请先突破' };
...
function applyAutoLevelUps(player, maxLevels = 1) {
  if (!player || typeof player !== 'object') return;
  const breakpoints = new Set(BREAKTHROUGH_LEVELS);
  let left = Math.max(0, Math.floor(Number(maxLevels) || 0));
  while (left > 0 && (Number(player.level) || 1) < 320 && !breakpoints.has(Number(player.level) || 1)) {
    const curLv = Number(player.level) || 1;
    if (curLv >= 320) break; // 道祖大圆满封顶
    const lv = Number(player.level) || 1;
    const exp = Number(player.exp) || 0;
...
// 突破辅助丹药配置：每个境界对应一种丹药，服用后累积突破成功率
const BREAKTHROUGH_PILL_CONFIG = {
  foundation: { itemId: 1,   storedField: 'breakthrough_foundation_pills_stored', cap: 5, bonusPerPill: 0.20, label: '筑基丹' },
  yunling:    { itemId: 5,   storedField: 'breakthrough_yunling_stored',         cap: 4, bonusPerPill: 0.20, label: '蕴灵丹' },
  yuanying:   { itemId: 352, storedField: 'breakthrough_yuanying_stored',        cap: 3, bonusPerPill: 0.20, label: '元婴育神丹' },
  huashen:    { itemId: 353, storedField: 'breakthrough_huashen_stored',         cap: 3, bonusPerPill: 0.20, label: '化神淬体丹' },
  lianxu:     { itemId: 354, storedField: 'breakthrough_lianxu_stored',          cap: 3, bonusPerPill: 0.20, label: '炼虚合道丹' },
  heti:       { itemId: 290, storedField: 'breakthrough_heti_stored',            cap: 3, bonusPerPill: 0.20, label: '合体归元丹' },
  dacheng:    { itemId: 291, storedField: 'breakthrough_dacheng_stored',         cap: 2, bonusPerPill: 0.25, label: '大乘渡劫丹' },
  zhenxian:   { itemId: 300, storedField: 'breakthrough_zhenxian_stored',        cap: 2, bonusPerPill: 0.25, label: '飞升接引丹' },
  jinxian:    { itemId: 310, storedField: 'breakthrough_jinxian_stored',         cap: 2, bonusPerPill: 0.25, label: '金仙证道丹' },
  taiyi:      { itemId: 320, storedField: 'breakthrough_taiyi_stored',           cap: 1, bonusPerPill: 0.30, label: '太乙归元丹' },
  daluo:      { itemId: 355, storedField: 'breakthrough_daluo_stored',           cap: 1, bonusPerPill: 0.30, label: '大罗混元丹' },
  daozu:      { itemId: 356, storedField: 'breakthrough_daozu_stored',           cap: 1, bonusPerPill: 0.30, label: '道祖证道丹' },
};
...
// ── 十三大境界突破节点（人界·灵界·仙界三层分割，共 12 个突破点） ──────────────
// 等级映射：炼气 1-120（13 层）→ 筑基 121-136 → 结丹 137-152 → 元婴 153-168 →
// 化神 169-184 → 炼虚 185-200 → 合体 201-216 → 大乘 217-232 → 真仙 233-248 →
// 金仙 249-264 → 太乙 265-280 → 大罗 281-296 → 道祖 297-320（封顶）
const BREAKTHROUGH_NODES = [
  { at: 120, to: 121, realm: '筑基',   base: 0.20, pills: 'foundation' },
  { at: 136, to: 137, realm: '结丹',   base: 0.16, pills: 'yunling' },
  { at: 152, to: 153, realm: '元婴',   base: 0.14, pills: 'yuanying', kill: true },
  { at: 168, to: 169, realm: '化神',   base: 0.12, pills: 'huashen', dungeon: true },
  { at: 184, to: 185, realm: '炼虚',   base: 0.10, pills: 'lianxu' },
  { at: 200, to: 201, realm: '合体',   base: 0.08, pills: 'heti' },
  { at: 216, to: 217, realm: '大乘',   base: 0.06, pills: 'dacheng' },
  { at: 232, to: 233, realm: '真仙',   base: 0.05, pills: 'zhenxian' },
  { at: 248, to: 249, realm: '金仙',   base: 0.04, pills: 'jinxian' },
  { at: 264, to: 265, realm: '太乙',   base: 0.03, pills: 'taiyi' },
  { at: 280, to: 281, realm: '大罗',   base: 0.02, pills: 'daluo' },
  { at: 296, to: 297, realm: '道祖',   base: 0.01, pills: 'daozu' },
];
const BREAKTHROUGH_LEVELS = BREAKTHROUGH_NODES.map((n) => n.at);

/** 灵根品质对突破成功率的加成：天灵根/变异灵根显著提速，伪灵根修炼艰难 */
function getSpiritRootQualityBonus(player) {
  const q = String((player && player.spirit_root_quality) || 'zhen');
  const map = { tian: 0.15, variant: 0.20, zhen: 0.05, wei: -0.05 };
  return Object.prototype.hasOwnProperty.call(map, q) ? map[q] : 0;
}

/** 灵根品质对修炼速度（经验获取）的倍率 */
function getSpiritRootExpMult(player) {
  const q = String((player && player.spirit_root_quality) || 'zhen');
  const map = { tian: 1.25, variant: 1.35, zhen: 1.0, wei: 0.85 };
  return Object.prototype.hasOwnProperty.call(map, q) ? map[q] : 1.0;
}
...
function breakthrough(player) {
  ...
  const node = BREAKTHROUGH_NODES.find((n) => lv === n.at);
  if (!node) return { ok: false, error: '当前等级无法突破' };
  return breakthroughRealm(player, node);
}

function breakthroughRealm(player, node) {
  const expNeeded = calculateExpNeeded(node.at);
  if (Number(player.exp) < expNeeded) return { ok: false, error: '经验不足' };

  let rate = node.base;
  // 灵根品质加成
  rate += getSpiritRootQualityBonus(player);

  // 突破辅助道具加成（通用逻辑）
  if (node.pills) {
    const pillCfg = BREAKTHROUGH_PILL_CONFIG[node.pills];
    if (pillCfg) {
      const stored = Number(player[pillCfg.storedField]) || 0;
      if (stored > 0) {
        const effective = Math.min(stored, pillCfg.cap);
        rate += effective * pillCfg.bonusPerPill;
      }
      player[pillCfg.storedField] = 0;
    }
  }

  // 特殊成就加成：元婴（击杀首领）、化神（挑战秘境）
  if (node.kill) {
    rate += Math.min(0.45, (Number(player.breakthrough_nascent_kill_count) || 0) * 0.003);
  }
  if (node.dungeon) {
    rate += Math.min(0.30, (Number(player.breakthrough_spirit_dungeon_count) || 0) * 0.03);
  }

  // 六维基础属性加成：越深厚，突破越稳
  const sixStats = getBaseSixStats(player);
  if (sixStats > 0) {
    const attrRate = Math.min(0.25, sixStats / (node.at * 12));
    if (attrRate > 0) rate += attrRate;
  }
  ...
  if (roll < rate) {
    player.level = node.to;
    grantTalentPointsForLevel(player);
    recalcAndAssignCombatStats(player);
    return { ok: true, player, success: true, realm: node.realm };
  }
  if (node.kill) player.breakthrough_nascent_kill_count = 0;
  if (node.dungeon) player.breakthrough_spirit_dungeon_count = 0;
  const penaltyInfo = _applyBreakthroughPenalty(player, node.at);
  return { ok: true, player, success: false, penalty: penaltyInfo, realm: node.realm };
}
...
function useItem(player, page, slotIndex, count = 1, useOptions = null) {
  ...
  // 突破辅助丹药：通用服用逻辑
  const pillEntry = Object.entries(BREAKTHROUGH_PILL_CONFIG).find(([, cfg]) => cfg.itemId === itemId);
  if (pillEntry) {
    const [, pillCfg] = pillEntry;
    const cap = pillCfg.cap;
    const stored = Number(player[pillCfg.storedField]) || 0;
    if (stored >= cap) return { ok: false, error: `${pillCfg.label}已服用满 ${cap} 枚` };
    const cnt = Number(slot.count) || 1;
    const useAmt = Math.min(Math.max(1, Math.floor(Number(count) || 1)), cnt, cap - stored);
    player[pillCfg.storedField] = stored + useAmt;
    consumeFromSlot(useAmt);
    const pct = Math.round(pillCfg.bonusPerPill * 100);
    return { ok: true, player, used_count: useAmt, msg: `已服用${useAmt}枚${pillCfg.label} (${player[pillCfg.storedField]}/${cap})，每枚+${pct}%成功率` };
  }
...
Object.assign(module.exports, {
  ...
  breakthrough,
  getSpiritRootExpMult,
  getSpiritRootQualityBonus,
  useItem,
  equipSkill,
  unequipSkill,
  setKeySkill,
