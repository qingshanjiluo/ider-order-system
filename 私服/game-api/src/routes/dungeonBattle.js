/**
 * 副本多人战斗 / 城池斗法 API（Worker 版）
 * 迁移自 server/routes/dungeonBattle.js。
 * - POST /dungeon-battle/start    创建副本战斗（支持组队）
 * - POST /dungeon-battle/advance  推进一步（服务端处理下一个行动者）
 * - GET  /dungeon-battle/city_duel/list     斗法目标列表
 * - GET  /dungeon-battle/city_duel/inspect  目标装备/属性查看
 * - GET  /dungeon-battle/city_duel/logs     斗法战报
 * - GET  /dungeon-battle/city_duel/rank     战神榜
 * - POST /dungeon-battle/city_duel/start    发起斗法
 *
 * 偏差：原 route 依赖 settlementLock（进程内写锁）与 setInterval 定时清理，
 *       本版本省略写锁（D1 原子操作保障），战斗状态写合并缓存使用模块级内存缓存，
 *       斗法赛季结算改由 rank 访问时惰性触发（无 Cron）。
 */
import { createDb } from '../db.js';
import { verifyToken } from '../auth.js';
import { getDungeonById, getDungeonEnemyById, getSectById, getItemById, getItems, getSkillById, getDungeons } from '../game/dataLoader.js';
import * as EG from '../game/equipmentGen.js';
import * as CU from '../game/combatUtils.js';
import * as CD from '../game/combatDamage.js';
import * as ops from '../game/playerOps.js';
import * as cave from '../game/cave.js';
import * as engine from '../game/dungeonBattleEngine.js';
import * as UTC from '../game/universalTime.js';
import * as dungeonBattleCache from '../game/dungeonBattleCache.js';
import { getNextSettlementTs, getCurrentPeriodIndex, pickPeriodReward, createDuelRankSeason } from '../game/duelRankSeason.js';
import { checkCommandRate, getCommandDelay } from '../game/commandRateLimit.js';
import {
...
import { settleKillTaskProgress } from './online.js';

function json(body, status = 200) {
  return new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json' } });
}

const DUNGEON_ADVANCE_EVENT_LIMIT = 60;
...
function _isLiteStateRequested(url) {
  const mode = String(url?.searchParams?.get('state') || '').trim().toLowerCase();
...
function getDateKeySafe(db) {
  if (db && typeof db.getDateKey === 'function') {
...
function clampRestUntil(player, nowSecV) {
  if (!player || typeof player !== 'object') return 0;
  const now = intVal(nowSecV, Math.floor(Date.now() / 1000));
...
function applyBattleStartTalismanToDungeonState(state, talismanUse) {
  ...
  for (const eff of effects) {
    ...
    if (type === 'battle_start_apply_slow') {
      ...
      for (const e of state.enemies) {
        if (!e || !e.alive || Number(e.hp) <= 0) continue;
        CU.applyDebuff(e, { type: 'chanfu', stacks: dur, damagePercent: dmgPct, attribute: 'spell_attack' }, state, caster);
      }
...
function addFormationDropItem(player, item, count, drops, nowSecV = Math.floor(Date.now() / 1000)) {
  if (!item || !item.id) return false;
  const c = Math.max(1, intVal(count, 1));
  const stored = cave.addFormationItems(player, clone(item), c, nowSecV);
...
function buildDungeonClearDrops(dungeonQuality, player, dropMult = 1) {
  ...
  if (Math.random() < 0.5) {
    ...
    const affixCap = EG.getPlayerAffixQualityCap(intVal(player.level, 1));
    const equipCount = Math.max(1, randomInt(1, 2) * mult);
    for (let i = 0; i < equipCount; i += 1) {
      const template = pick(equipPool);
      if (!template) break;
      const eq = EG.rollEquipmentFromTemplateItem(template, affixCap) || template;
...
function buildFormationDungeonRuneDrops(dungeon, player) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    addFormationDropItem(player, item, 1, drops, nowSecV);
  }
  return { drops };
}
...
function createCityDuelEnemySnapshotFromPlayer(player, accountId) {
  ...
  const snap = CU.buildPlayerSnapshot(source, { skipInventory: true, battleMode: 'city_duel', isTeamBattle: false });
  return {
    ...snap,
...
async function _buildDuelList(db) {
...
async function _buildDuelRank(db) {
  const now = Math.floor(Date.now() / 1000);
...
function _extractIntQuery(url, key, d = 0) {
  // searchParams.get 缺失时返回 null（Number(null)===0），需视为未传使用默认值
  const v = url?.searchParams?.get(key);
  return intVal(v == null ? undefined : v, d);
}

export async function handleDungeonBattleRoute(request, env, route) {
  const method = request.method;
  const url = new URL(request.url);
  const subPath = route.replace(/^\/dungeon-battle/, '') || '/';
  ...
  // 注入当前请求的落盘函数（缓存 flush 使用）
  dungeonBattleCache.init((battleId, aid, did, state) => db.saveDungeonBattle(battleId, aid, did, state));

  if (subPath === '/start' && method === 'POST') return handleDungeonStart(request, db, accountId);
  if (subPath === '/advance' && method === 'POST') return handleDungeonAdvanceRoute(request, db, accountId, url);
  if (subPath === '/city_duel/list' && (method === 'GET' || method === 'POST')) return handleCityDuelList(request, db, accountId, url);
  if (subPath === '/city_duel/inspect' && (method === 'GET' || method === 'POST')) return handleCityDuelInspect(request, db, accountId, url);
  if (subPath === '/city_duel/logs' && (method === 'GET' || method === 'POST')) return handleCityDuelLogs(request, db, accountId, url);
  if (subPath === '/city_duel/rank' && (method === 'GET' || method === 'POST')) return handleCityDuelRank(request, db, accountId, url, env);
  if (subPath === '/city_duel/start' && (method === 'GET' || method === 'POST')) return handleCityDuelStart(request, db, accountId);

  return null;
}

async function handleCityDuelList(request, db, accountId, url) {
  const meId = intVal(accountId, 0);
  const page = Math.max(1, _extractIntQuery(url, 'page', 1));
  const pageSize = Math.max(10, Math.min(100, _extractIntQuery(url, 'page_size', 30)));
  const keywordRaw = String(url?.searchParams?.get('keyword') || '').trim();
  const keyword = keywordRaw.toLowerCase();

  let list = (await _buildDuelList(db)).filter(e => e.account_id !== meId);
  if (keyword.length > 0) {
    list = list.filter((e) => {
...
async function handleCityDuelInspect(request, db, accountId, url) {
  const targetId = _extractIntQuery(url, 'target_account_id', 0);
  if (targetId <= 0) return json({ ok: false, error: '无效目标' });
  const target = await db.getPlayerByAccountId(targetId);
  if (!target) return json({ ok: false, error: '目标玩家不存在' });
  CU.recalcAndAssignCombatStats(target);
...
async function handleCityDuelLogs(request, db, accountId, url) {
  const page = Math.max(1, _extractIntQuery(url, 'page', 1));
  const pageSize = Math.max(1, Math.min(100, _extractIntQuery(url, 'page_size', 20)));
  const roleRaw = String(url?.searchParams?.get('role') || 'all').toLowerCase();
  const role = (roleRaw === 'challenger' || roleRaw === 'target') ? roleRaw : 'all';
  const result = await db.listCityDuelLogsByAccount(accountId, { page, pageSize, role });
...
async function handleCityDuelRank(request, db, accountId, url, env) {
  // 惰性触发赛季结算（原 server 由 Cron 负责，Worker 无长驻进程）
  try {
    const season = createDuelRankSeason(env);
    await season.trySettleIfDue();
  } catch (e) {
    console.error('[dungeon-battle/city_duel/rank] 赛季结算异常:', e?.message || e);
  }

  const limit = Math.min(100, Math.max(1, _extractIntQuery(url, 'limit', 50)));
  const list = await _buildDuelRank(db);
  const meId = intVal(accountId, 0);
...
async function handleCityDuelStart(request, db, accountId) {
  try {
    const body = await request.json().catch(() => ({}));
    const targetAccountId = intVal(body?.target_account_id, 0);
    if (targetAccountId <= 0) return json({ ok: false, error: '目标参数无效' });
    if (targetAccountId === intVal(accountId, 0)) return json({ ok: false, error: '不能挑战自己' });
    const playerRaw = await db.getPlayerByAccountId(accountId);
    if (!playerRaw) return json({ ok: false, error: '角色不存在' });
    ...
    if (targetLevel < myLevel - 10) {
      return json({ ok: false, error: `不能挑战比自己低10级以上的目标（你Lv.${myLevel}，对方Lv.${targetLevel}）` });
    }
    const nowSec = Math.floor(Date.now() / 1000);
    const restUntil = clampRestUntil(playerRaw, nowSec);
    if (restUntil !== intVal(playerRaw.rest_until, 0)) {
      playerRaw.rest_until = restUntil;
      await db.savePlayer(accountId, 1, playerRaw);
    }
    if (restUntil > nowSec) {
      const remain = Math.max(0, restUntil - nowSec);
      return json({ ok: false, error: `调息中，剩余${remain}秒`, rest_remaining_sec: remain });
    }
    const presetApplied = ops.tryApplySkillPresetForBattle(playerRaw, 'duel');
    cave.settleMainFormationServices(playerRaw, nowSec);
    if (presetApplied) await db.savePlayer(accountId, 1, playerRaw);
    const { total, perTarget } = await db.countCityDuelChallengesToday(accountId, targetAccountId);
    if (perTarget >= 3) return json({ ok: false, error: '今日对该玩家挑战次数已达上限（同一目标每天最多3次）' });
    const rankAffected = total < 5;
    const player = buildFullStateBattlePlayerSource(playerRaw, accountId);
    ...
    state.duel_challenger_account_id = intVal(accountId, 0);
    state.duel_challenger_name = String(playerRaw?.name || `道友#${intVal(accountId, 0)}`);
    ...
    dungeonBattleCache.removeAllForAccount(accountId);
    await db.deleteAllDungeonBattlesForAccount(accountId);
    await db.insertCityDuelChallenge(accountId, targetAccountId);
    await db.savePlayer(accountId, 1, playerRaw);
    await db.saveDungeonBattle(battleId, accountId, 0, state);
    dungeonBattleCache.save(battleId, accountId, 0, state);
    ...
    return json({ ok: false, error: '斗法初始化失败' }, 500);
  }
}
...
async function handleDungeonStart(request, db, accountId) {
  try {
    const body = await request.json().catch(() => ({}));
    const { dungeon_id, team_code } = body || {};
    const challengeMode = String(body?.challenge_mode || 'normal').trim().toLowerCase();
    const isTrialContractMode = challengeMode === 'trial_contract';
    const contractModifierIds = isTrialContractMode
      ? normalizeSelectedTrialContracts(body?.contract_modifiers, 6)
      : [];
    const modeRaw = String(body?.dungeon_mode || 'normal').trim().toLowerCase();
    const dungeonMode = modeRaw === 'formation' ? 'formation' : 'normal';
    const dungeonId = Number(dungeon_id);
    if (!dungeonId || dungeonId <= 0) return json({ ok: false, error: '无效的副本ID' });

    if (isTrialContractMode && team_code) {
      return json({ ok: false, error: '危机试炼仅支持单人挑战' });
    }
    if (isTrialContractMode && dungeonMode === 'formation') {
      return json({ ok: false, error: '危机试炼暂不支持阵法副本' });
    }
    ...
    if (!isTrialContractMode) {
      const completions = await db.getDungeonCompletionsToday(accountId, dungeonId);
      const dailyLimit = Number(dungeon.daily_limit) || 2;
      if (completions >= dailyLimit) return json({ ok: false, error: '今日次数已用完' });
    }
    ...
    if (!player) return json({ ok: false, error: '角色不存在' });
    const nowSec = Math.floor(Date.now() / 1000);
    ...
    if (presetApplied) await db.savePlayer(accountId, 1, player);
    allyPlayers.push(buildFullStateBattlePlayerSource(player, accountId));
    ...
    if (waves.length === 0) return json({ ok: false, error: '副本没有怪物配置' });
    ...
    if (talismanUse.used) applyBattleStartTalismanToDungeonState(state, talismanUse);
    await db.saveDungeonBattle(battleId, accountId, dungeonId, state);
    dungeonBattleCache.save(battleId, accountId, dungeonId, state);
    ...
    return json({ ok: false, error: '副本初始化失败' }, 500);
  }
}

async function handleDungeonAdvanceRoute(request, db, accountId, url) {
  const _delay = getCommandDelay(accountId);
  if (_delay > COMMAND_MAX_QUEUE_WAIT_MS) {
    return _respondDungeonAdvanceThrottled(request, db, accountId, _delay, url);
  }
  if (_delay > 0) await new Promise((resolve) => setTimeout(resolve, _delay));
  return _handleDungeonAdvance(request, db, accountId, url);
}

async function _respondDungeonAdvanceThrottled(request, db, accountId, delayMs, url) {
  const liteState = _isLiteStateRequested(url);
  const body = await request.json().catch(() => ({}));
  ...
  if (!battle || Number(battle.account_id) !== Number(accountId) || !battle.state || typeof battle.state !== 'object') {
    return json({
      ok: true,
      throttled: true,
      ended: true,
...
async function _handleDungeonAdvance(request, db, accountId, url) {
  try {
    const liteState = _isLiteStateRequested(url);
    ...
    const battle = dungeonBattleCache.get(battle_id) || await db.getDungeonBattle(battle_id);
    if (!battle || Number(battle.account_id) !== Number(accountId)) {
      return json(battleGoneResp);
    }
    ...
    if (result.ended) {
      dungeonBattleCache.remove(battle_id);
      await db.deleteDungeonBattle(battle_id);
      const p = await db.getPlayerByAccountId(accountId);
      if (battleMode === 'city_duel') {
        ...
        const challengerId = intVal(st.duel_challenger_account_id, intVal(accountId, 0));
          ...
          if (!isDraw && st.duel_rank_affected !== false) {
            ...
            for (const [aid, isWinner] of [[challengerId, winnerId === challengerId], [targetId, winnerId === targetId]]) {
              ...
              if (pp) {
                ...
                if (aid === intVal(accountId, 0) && p) p.duel_rank_score = pp.duel_rank_score;
              }
            }
          }
        }
        if (p) {
          ...
          await db.savePlayer(accountId, 1, p);
          resp.player = p;
        }
        ...
        if (!isTrialContract) {
          const doneCount = await db.getDungeonCompletionsToday(accountId, battle.dungeon_id);
          ...
          await db.incrementDungeonCompletions(accountId, battle.dungeon_id);
        }
        ...
        if (p) {
          ...
          let clearDrops = [];
          const todayKey = getDateKeySafe(db);
      ...
      dungeonBattleCache.save(battle_id, accountId, battle.dungeon_id, result.state);
    }
    ...
    return json({ ok: false, error: '战斗推进异常' }, 500);
  }
}